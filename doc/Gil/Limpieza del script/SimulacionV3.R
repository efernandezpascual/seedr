setwd ("C:/EFP/Trabayu/Activos/2018 FICYT3/GT2 R tool/Gil/Limpieza del script")
source("C:/EFP/Trabayu/Activos/2018 FICYT3/GT2 R tool/Gil/Limpieza del script/funciones.R")

library(data.table)


# Experiment Germination simulation
# PG is fixed for each Dish: Number of seeds per treatment = PG*Dish
# Time: Vector of observation times (censored times)
# Treatments: Treatments to be applied (hidrotime model)
# Sample: Generating function
#    - Normal: Sample=function(x){rnorm(x,mean=-1,sd=1)}
#    - Gamma: Sample=function(x){-rgamma(x,shape=1,scale=1)}
# ExtraInfo: Should extra simulation info be retrieved?
...
Sample=function(x){-rweibull(x,2,1)}, PG=25, Dish=4, ExtraInfo=TRUE)
{
  x<-Treatments
  k<-length(x)
...
#   shape<-2
#   scale<-1
#   Xb<- -rweibull(k*PG*Dish,shape,scale) # +0.4 para inducir dormición, -0.4 para que varios tratamientos lleguen al 100%
#   Xb<- rep(-qweibull(seq(0.005,0.995,length.out=PG*Dish),shape,scale),k)

  Xb<-Sample(k*PG*Dish)

#   data<-data.frame(Treatment=rep(x,each=PG*Dish),Dish=NA,Xb=Xb,G=NA,PG=PG,Tx=NA)        
r  data<-data.table(Treatment=rep(x,each=PG*Dish),Dish=NA,Xb=Xb,G=NA,PG=PG,Tx=NA)        r
r  data$Dish<-factor(LETTERS[rep(rep(1:Dish,each=PG),times=k)])
  ...
  dat<-data[,list(Time=Time,G=cumsum(as.numeric(freq(cut(.SD$Tx,c(0,Time)),type="ni",with.na=FALSE))),PG=rep(PG[1],length(Time))),by=list(Treatment,Dish)]
  sim<-dat
...
#       nr<-length(fr)
#       dat<-rbind(dat,data.frame(Treatment=Ds$Treatment[1],Dish=Ds$Dish[1],Time=Time,G=cumsum(as.numeric(fr)),PG=PG))   
#     }
#   }
#   sim=dat
...
# Get sample distribution functions for each treatment based on sampled data
GerminationTT2<-function(ddt)
{
  x<-as.data.table(ddt)
  y<-x[,.(PG=max(PG)),by=list(Dish,Treatment)]
  PG<-y[,.(PG=sum(PG)),by=Treatment]
    
  x<-x[,rbind(.(Time=c(0,0), G=c(0,0), PG=c(NA,NA)),.SD),by=list(Dish,Treatment)]

  x<-x[,.(Time=tail(Time,.N-1), ni=tail(G,.N-1)-head(G,.N-1)),by=list(Dish,Treatment)]
  x<-x[,.(ni=sum(ni),Prob=PG$PG[PG$Treatment==Treatment]),by=list(Time,Treatment)]

  x[,.(Time,ni,Prob=cumsum(ni)/Prob),by=list(Treatment)]
}

mapprox<-function(x,y,xout)
{
  if (max(x)-min(x)<1E-10) return(list(x=xout,y=rep(as.numeric(NA),times=length(xout))))
  approx(x,y,xout)
}
...
#   it<-it[it$Prob>0,]
#   it<-it[it$ni>0,]
  ii<-it[,cbind(mapprox(.SD$Prob,.SD$Time,it$Prob),data.frame(Time=it$Time,Treat=it$Treatment,ni=it$ni)),by=Treatment]
  ii<-ii[ii$Treatment>ii$Treat,]
  ratio<-(1-ii$y/ii$Time)
  ii$Theta<-(ii$Treatment-ii$Treat)*ii$y/ratio
  ii[is.finite(ii$Theta)&(ii$Theta>0)&(ii$ni>0)&(ratio>1E-10)]
}


#########################################################################################################
### One-core simulation (for Windows, otherwise more complex implementation of parallelization is needed
start_time <- Sys.time()
aa<-replicate(1000,{
  sm<-SimulateGerminatio(Ptheta=100, PG=25, ExtraInfo=FALSE)#, Treatments=c(-1.2,-1,-0.8,-0.6,-0.4,-0.2,0))  
  it<-GerminationTT2(sm$sim)
  mbDT<-ModelBasedThetaV4(it)
  c(mediaDT=media(mbDT$Theta), medianaDT=mediana(mbDT$Theta))
})
Sys.time()-start_time
...
#########################################################################################################
### Parallel simulation (for Linux, SO fork is automatically used)
library(parallelMap)
library(parallel)

sim<-function(x)
{
#  sm<-SimulateGerminatio(Ptheta=100, PG=25, ExtraInfo=FALSE, Sample=function(x){rnorm(x,-1,0.5)})#, Treatments=c(-1.2,-1,-0.8,-0.6,-0.4,-0.2,0))  
  sm<-SimulateGerminatio(Ptheta=100, PG=25, ExtraInfo=FALSE)
  it<-GerminationTT2(sm$sim)
  mbDT<-ModelBasedThetaV4(it)
  data.frame(mediaDT=media(mbDT$Theta), medianaDT=mediana(mbDT$Theta))
}

start_time <- Sys.time()
# aa<-mclapply(1:1000, sim, mc.cores =detectCores()-1)
# aa<-do.call(rbind,mclapply(1:100, sim, mc.cores =detectCores()-1))
aa<-rbindlist(mclapply(1:1000, sim, mc.cores =detectCores()-1))
Sys.time()-start_time
estadisticos.media(as.data.frame(aa))
#########################################################################################################
...
cdf<-function(data, Interpolated=TRUE)
{
  ...
  else return(list(cdf=cfd,cdfInt=data.frame()))
}
...
GerminationTT<-function(ddt, Interpolated=TRUE)
{
...
ModelBasedThetaV2<-function(germ) #GmP,GerminationList)
{
  GmP<-germ$Base
  GerminationList<-germ$GerminationList
  ...
  for (i in 1:l)
  {
    for (j in 1:k)
    {
      ...
      {
        ID<-c(ID,i)
        Theta<-c(Theta,((as.numeric(names(GerminationList)[i])-GmP$Treatment[j]))*quantileCFD(GerminationList[[i]],prob=GmP$gamma[j])/(1-1/maximo(germ$GerminationList[[i]]$Time)))
      }
    }
  }
  ret<-data.frame(ID=ID,Theta=Theta)
...
ModelBasedThetaV3<-function(germ) 
{
  GerminationList<-germ$GerminationList
  ...
  for (i in 1:l)
  {
    Gm<-GerminationList[[i]][GerminationList[[i]]$Prob>0,]
    Gm<-GerminationList[[i]][GerminationList[[i]]$ni>0,]
    k<-nrow(Gm)
    if (k>0)
    {
      for (s in 1:k)
      {
        ...
        for (j in 1:l)
        {
          if ((j!=i) & (max(GerminationList[[j]]$Prob)>=g) & (as.numeric(names(GerminationList)[j])-as.numeric(names(GerminationList)[i]))>0) 
          {
            qqu<-quantileCFD(GerminationList[[j]],prob=g)
            if (qqu<T)          
            {
              Theta<-c(Theta,((as.numeric(names(GerminationList)[j])-as.numeric(names(GerminationList)[i])))*qqu/(1-qqu/T))
              ID<-c(ID,as.numeric(names(GerminationList)[j]))
              ID2<-c(ID2,as.numeric(names(GerminationList)[i]))
              IDp<-c(IDp,g)
              IDq<-c(IDq,quantileCFD(GerminationList[[j]],prob=g))
              IDt<-c(IDt,T)
            }
          }
        }
  ...
  ret<-data.frame(ID=ID,ID2=ID2,IDp=IDp,IDq=IDq,IDt=IDt,Theta=Theta)
  ret
#  by.groups(ret$Theta,abs(ret$ID-ret$ID2),estadisticos.media)
  ret[ret$Theta>0,]
}
...
ModelBasedThetaV3<-function(germ) 
{
  ...
  for (i in 1:l)
  {
    ii<-as.numeric(names(GerminationList)[i])
    Gm<-GerminationList[[i]][GerminationList[[i]]$ni>0,]
    k<-nrow(Gm)
    if (k>0)
    {
      for (s in 1:k)
      {
        ...
        for (j in 1:l)
        {
          jj<-as.numeric(names(GerminationList)[j])
          if ((j!=i) & (max(GerminationList[[j]]$Prob)>=g) & (jj-ii)>0) 
          {
            qqu<-quantileCFD(GerminationList[[j]],prob=g)
            if (qqu<T)          
            {
              Theta<-c(Theta,((jj-ii))*qqu/(1-qqu/T))
              ID<-c(ID,jj)
              ID2<-c(ID2,ii)
              IDp<-c(IDp,g)
              IDq<-c(IDq,qqu)
...
SimulateGerminatio<-function(Treatments=c(-1.6,-1,-0.8,-0.6,-0.4,-0.2,0), Ptheta=33, Time=c(7,14,18,21,28,33,37,41,47,52,57,62,76,89,101,111,126,135,151,197,341,532),
Sample=function(x){-rweibull(k*PG*Dish,2,1)}, PG=25, Dish=4, ExtraInfo=TRUE)
{
  x<-Treatments
  k<-length(x)
...
#   data<-data.frame(Treatment=rep(x,each=PG*Dish),Dish=NA,Xb=Xb,G=NA,PG=PG,Tx=NA)        
  data<-data.table(Treatment=rep(x,each=PG*Dish),Dish=NA,Xb=Xb,G=NA,PG=PG,Tx=NA)        
  data$Dish<-factor(LETTERS[rep(rep(1:Dish,each=PG),times=k)])
  data$Tx<-Ptheta/(data$Treatment-Xb)
  data$Tx[data$Tx<0]<-NA
...
sm<-SimulateGerminatio(Ptheta=33)  
dat<-sm$sim
data<-sm$dtorig
data[101:200,]
...
start_time <- Sys.time()
aa<-replicate(1000,{
 sm<-SimulateGerminatio(Ptheta=1, PG=25)  
 germ<-GerminationTT(sm$sim)
 
 mb<-ModelBasedThetaV3(germ)
 mb1<-ModelBasedTheta(germ$Base,germ$GerminationList)
 mb2<-ModelBasedThetaV2(germ)
#  c(media1=media(ModelBasedThetaV2(germ))$Theta, media2=media(mb)$Theta,mediaC=mediana(mb)$Theta, minCV=MinCVTheta(germ), minVQ=MinVarQuantile(germ,removeQ=NA)$Theta)
 c(media1=media(mb1$Theta), media2=media(mb2$Theta), media=media(mb$Theta), mediana=mediana(mb$Theta))
})
Sys.time()-start_time
estadisticos.media(as.data.frame(t(aa)))
...
sim<-function(x)
{
 sm<-SimulateGerminatio(Ptheta=50, PG=25, ExtraInfo=FALSE)  
 germ<-GerminationTT(sm$sim, Interpolated=FALSE)
...
#  c(media1=media(ModelBasedThetaV2(germ))$Theta, media2=media(mb)$Theta,mediaC=mediana(mb)$Theta, minCV=MinCVTheta(germ), minVQ=MinVarQuantile(germ,removeQ=NA)$Theta)
# c(media1=media(mb1$Theta), media2=media(mb2$Theta), media=media(mb$Theta), mediana=mediana(mb$Theta))
   data.frame(media1=media(mb1$Theta), media=media(mb$Theta), mediana=mediana(mb$Theta))
}
...
ddt<-as.data.table(ddt)


###########
library(data.table)
GerminationTT2<-function(ddt, Interpolated=TRUE)
{
  x<-as.data.table(ddt)
  y<-x[,.(PG=max(PG)),by=list(Dish,Treatment)]
  PG<-y[,.(PG=sum(PG)),by=Treatment]
...
# data.frame(media1=media(mb$Theta), mediana1=mediana(mb$Theta))
c(mediaDT=media(mbDT$Theta), medianaDT=mediana(mbDT$Theta))
})
Sys.time()-start_time
estadisticos.media(as.data.frame(t(aa)))
...
#  c(media1=media(ModelBasedThetaV2(germ))$Theta, media2=media(mb)$Theta,mediaC=mediana(mb)$Theta, minCV=MinCVTheta(germ), minVQ=MinVarQuantile(germ,removeQ=NA)$Theta)
# c(media1=media(mb1$Theta), media2=media(mb2$Theta), media=media(mb$Theta), mediana=mediana(mb$Theta))
# data.frame(media1=media(mb$Theta), mediana1=mediana(mb$Theta), mediaDT=media(mbDT$Theta), medianaDT=mediana(mbDT$Theta))
# data.frame(media1=media(mb$Theta), mediana1=mediana(mb$Theta))
data.frame(mediaDT=media(mbDT$Theta), medianaDT=mediana(mbDT$Theta))
}
...
germ<-GerminationTT(sm$sim, Interpolated=FALSE)
ModelBasedThetaV3(germ)

sm<-SimulateGerminatio(Ptheta=50, PG=25, ExtraInfo=FALSE)  
it<-GerminationTT2(sm$sim)

dgg<-get.groups(dg[[i]],dg[[i]]$Dish)

cdf(ddt,Interpolate=FALSE)

start_time <- Sys.time()
aaa<-replicate(100,GerminationTT(sm$sim,Interpolated=FALSE))
Sys.time()-start_time

start_time <- Sys.time()
aaa<-replicate(100,GerminationTT2(sm$sim))
Sys.time()-start_time
...
# it<-it[it$Prob>0,]
#it<-it[it$ni>0,]
ii<-it[,cbind(approx(.SD$Prob,.SD$Time,it$Prob),data.frame(Time=it$Time,Treat=it$Treatment,ni=it$ni)),by=Treatment]
ii<-ii[ii$Treatment>ii$Treat,]
ii$Theta<-(ii$Treatment-ii$Treat)*ii$y/(1-ii$y/ii$Time)
ii<-ii[is.finite(ii$Theta)&(ii$Theta>0)&(ii$ni>0)]

median(ii$Theta)

germ<-GerminationTT(sm$sim, Interpolated=FALSE)
jj<-ModelBasedThetaV3(germ)
jj[order(jj$ID,jj$ID2),]


median(jj$Theta)


it[,cbind(approx(.SD$Prob,.SD$Time,it$Prob),as.data.frame(it[,1:2])),by=Treatment]
...
MSD<-it[1:5,]
cbind(as.data.table(mapprox(x=MSD$Prob,y=MSD$Time,xout=it$Prob)),MSD)
cbind(as.data.table(approx(MSD$Prob,MSD$Time,it$Prob)),it$Time)

MSD<-it[24:46,]
cbind(as.data.table(approx(MSD$Prob,MSD$Time,MSD$Prob)),MSD)

rr<-mapprox(MSD$Prob,MSD$Time,it$Prob)
as.numeric(rr$y)
...
start_time <- Sys.time()
aaa<-replicate(1000,it[,approx(.SD$Prob,.SD$Time,it$Prob),by=Treatment])
Sys.time()-start_time

outer(it$Treatment,it$Treatment,"*")
...
start_time <- Sys.time()
aaa<-replicate(10000,approx(it$Prob,it$Time,(1:100)/100))
Sys.time()-start_time


cdf<-function(x, Interpolated=TRUE)
{
  x<-as.data.table(x)
  y<-x[,.(PG=max(PG)),by=Dish]
  PG<-sum(y$PG)
  x<-x[,rbind(.(Treatment=NA, Time=0, G=0, PG=NA),.SD),by=Dish]
  x<-x[,.(Time=tail(Time,.N-1), ni=tail(G,.N-1)-head(G,.N-1)),by=Dish]
  x<-x[,.(ni=sum(ni)),by=Time]
  x$Prob<-cumsum(x$ni/PG) 

  if (Interpolated)
  {
    pg<-seq(0,sum(x$ni)/PG,by=1/PG)
    return(list(cdf=x,cdfInt=data.frame(Time=quantileCFD(x,pmax(0,pg-(0.5/PG))), Prob=pg)))
  }
  else return(list(cdf=x,cdfInt=data.frame()))
}
...
x[,rbind(list(G=10),head(G,.N-1)),by=Dish]


DT = data.table(ID= 1:2, v = 3:4, v2 = 5:6); 
DT[, rbind(.(v = 0, v2 = .GRP), .SD), by=ID]

dgg[[2]]$G<-dgg[[2]]$G- c(0,dgg[[2]]$G[-nrow(dgg[[2]])])
  
rep(dgg[[2]]$Time,times=dgg[[2]]$G- c(0,dgg[[2]]$G[-nrow(dgg[[2]])]))
  
 by.groups 

DT[, .N, by=x]   

parallelStart("socket",cpus=detectCores()-1)
result <- parallelLapply(1:100,function(i)
{
 sm<-SimulateGerminatio(Ptheta=250)  
 germ<-GerminationTT(sm$sim)
 
 GmP<-germ$Base
 mb<-ModelBasedThetaV3(germ)
#  c(media1=media(ModelBasedTheta(GmP,germ$GerminationIntList))$Theta, media2=media(mb)$Theta,mediaC=mediana(mb)$Theta, minCV=MinCVTheta(germ), minVQ=MinVarQuantile(germ,removeQ=NA)$Theta)
 c(media1=media(ModelBasedTheta(GmP,germ$GerminationIntList))$Theta, media2=media(mb)$Theta,mediaC=mediana(mb)$Theta)
})
...
Theta<-media(ModelBasedThetaV2(germ))$Theta
Theta<-media(ModelBasedThetaV3(germ))$Theta

FXb2<-FTxToFXb(germ, Theta)

germ$GerminationList
