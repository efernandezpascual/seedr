...
physiotime <- function(d, t, g, pg, x, reps = NULL, groups = NULL, method)
{
  ...
  if(is.null(groups)) {
    ...
    listd <- split(d, by = groups, drop = TRUE)
    z <- lapply(listd, function(d) {
      zi <- get(method)(d, t, g, pg, x, reps)
      zi$groups <- physiogroups(d, groups)
      zi
      })
    class(z) <- "physiolist"}
  z
...
{
  d <- data.table(d)
  d[order(get(t)), cumulative := cumsum(get(g)), by = c(x, reps, groups)]
#  if(is.null(reps) & is.null(groups)) {
#    d[order(get(t)), cumulative := cumsum(get(g)), by = c(x)]
#  } else {
#    if(is.null(groups)) {
#      d[order(get(t)), cumulative := cumsum(get(g)), by = c(x, reps)]
#    } else {
#      if(is.null(reps)) {
#        d[order(get(t)), cumulative := cumsum(get(g)), by = c(x, groups)]
#      } else d[order(get(t)), cumulative := cumsum(get(g)), by = c(x, reps, groups)]
#    }
#  }
  d[, germination := cumulative / get(pg)]
}

#' @export
physiogroups <- function(d, groups = NULL)
{
  if(is.null(groups)) NULL
  else{
    g <- unique(d[, ..groups])
    g <- g[, lapply(.SD, as.character)]
    g[, group := do.call(paste, Map(function(x, y) paste(x, y, sep = ":"),
                                    names(.SD), .SD))]
  }
}
...
bradford <- function(d, t, g, pg, x, reps = NULL) tryCatch(
  {
    ...
    if(is.null(reps)) z$parameters$n.viable <- sum(d[, .(n = max(get(pg))),
                                                     by = .(get(x))][, n])
...
{
  a <- sapply(d, function(y) group = y$groups[,-ncol(y$groups), with = FALSE])
  b <- sapply(d, function(y) coef(y))
  z <- rbind(a, b)
  dfz <- data.frame(t(z))
  rownames(dfz) <- NULL
  dfz
