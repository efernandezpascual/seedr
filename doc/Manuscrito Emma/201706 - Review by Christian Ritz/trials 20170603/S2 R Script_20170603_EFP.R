...
# Write a function (GRfun) to fit dose-response models to cumulative germination vs time, i.e. gerimnation rate
...
# Several dose-response models are available and suitable for cumulative germination data (see the documentation of the package 'drc')
...
GRfun <- function(x) {
         ...
         m2 <- drm(G ~ Start + End, data = x, type = "event", fct = W1.2())
         m3 <- drm(G ~ Start + End, data = x, type = "event", fct = W2.2())
         m4 <- drm(G ~ Start + End, data = x, type = "event", 
