# An easy and automated calculation of the germination cardinal temperatures 
# and thermal time using R
...
dat1 <- read.table("Supplementary Information 2  5_2_17_EL.txt", header = T)
...
CTfun <- function(x) {
         ...
         segmented.mod <- segmented(lin.mod, seg.Z = ~ Treatment, psi = to)
...
# Outlying datapoints may suggest that some of the temperature treatments should be removed
# from the calculations. Look specially for points at the extremes that can affect greatly
...
# ADDITIONAL STEP for data exploration if the above model fitting fails

# Plot a smooth line figure for irregular data
# Warnings may be printed when the fit fails in some deciles, ignore them
...
# You can view the figure by running 'SMTHfig'
# Save and export SMTHfig
ggsave(filename = "Fig 5 Germination rate vs T (smooth model).tiff", plot = SMTHfig,
       path = NULL, scale = 1, width = 173, height = 173, units = "mm", dpi = 300)
