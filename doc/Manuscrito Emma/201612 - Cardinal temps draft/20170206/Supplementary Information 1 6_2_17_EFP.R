...
data <- read.table("Supplementary Information 2  5_2_17_EL.txt", header = T)
...
# Apply the function for every treatment in each grouping and call it 'FGP'
...
# Write a function to test the fit of different dose-response functions to your data
FS <- function(x) {
...
# Apply the above function to each 'Groupings' & 'Treatments'
FSfit <- ddply(data, .(Grouping, Treatment), failwith(f = FS, quiet = T))

# View the table 'FSfit', which tells you the best fit for each 'Groupings' & 'Treatments'
...
# for example, replacing it with 'fct="W1.4()'
...
# Then you apply the above function to each each 'Groupings' & 'Treatments'
...
# Set a function to fit the cumulative germination function and extract the germination rates
...
GR$Rates <- 1/GR$Times
# and add labels with the deciles
...
# Set a function to fit linear regression and calculate base or ceiling T
LMfun <- function(x) {
         lin.mod <- lm(Rates ~ Treatment, data = x)
